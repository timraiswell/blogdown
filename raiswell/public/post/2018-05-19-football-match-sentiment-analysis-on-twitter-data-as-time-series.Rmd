...
api_key <- "Your Info" # From dev.twitter.com
api_secret <- "Your Info" # From dev.twitter.com
token <- "Your Info" # From dev.twitter.com
token_secret <- "Your Info" # From dev.twitter.com
...
```{r}
#A useful example of a fan tweet
mufc_tweets[[119]] #Nested square parentheses help me access the tweets 
```

This tweet before the game starts is designed to attract the attention of Manchester United and other football fans by using the '#ManchesterUnited' hastag along with a media hashtag. It is problematic because it uses the word 'killing', which will be picked up in our sentiment analysis as a negative sentiment associated with Manchester United fans. 

```{r}
#A example of 'advertising' for Manchester United fan attention
mufc_tweets[[6]]
```

Now I convert the output to a dataframe, which is a more intuitive format for me to work with.
...
Now I use Posix strings that approximate pre- and post-game time periods to help me 'zoom in' on the sentiment I wanbt to examine. 
...
(mufc_game$text[109])
```
Excuse my printing the language - this tweet expresses frustration with a Manchester United player: Jesse Lingard.
```{r, echo = FALSE}
(mufc_game$text[41]) # 
```
And while this tweet expresses frustration with a television pundit, it is indicative of general Manchester United fan sentiment at this point in the game.
```{r, echo = FALSE}
(mufc_game$text[27]) # 
```
...
Let's take a look at the aggregated sentiment for the game in two ways. First we'll plot a histogram of fan sentiment that will help us to compare the teams side by side. And then we'll perform an analysis of variance to see if the differences in fan sentiment are statistically significant (for this small sample). 
...
The mean sentiment of Manchester United fans for the game period is **0.024**; for Yeovil fans it is **0.036**. Does this answer our question above? Are United fans more critical of their team and lkkess likely to enjoy a game, even if they win? Sentiment scores for both sets of fans fail a Shapiro-Wilk normality test, so we use a [non-parametric, two-sample Wilcoxon rank test](http://www.sthda.com/english/wiki/unpaired-two-samples-wilcoxon-test-in-r).  
